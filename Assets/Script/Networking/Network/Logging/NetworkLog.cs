...
namespace Network.Logging
{
    ...
    internal class NetworkLog
    {
        ...
        /// </summary>
        internal string TimeStamp => DateTime.Now.ToString("HH:mm:ss:fff");
        ...
        internal void Log(string message, Exception exception, LogLevel logLevel = LogLevel.Information)
        {
            ...
            var finalLogMessage = BuildLogHeader(exception, logLevel);
            ...
            var tableOutput = new ConsoleTable(tableColumnHeaders);
        ...
        internal void SetOutputStream(Stream stream)
        {
        ...
        {
            var localEndPoint = monitoredConnection.IPLocalEndPoint?.ToString();
            var remoteEndPoint = monitoredConnection.IPRemoteEndPoint?.ToString();

            object type = monitoredConnection.GetType().Name;
            object route = string.Format("{0} -> {1}",
                direction == PacketDirection.Incoming ? remoteEndPoint : localEndPoint,
