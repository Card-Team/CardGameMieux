...
namespace Network
{
    ...
    public class ClientConnectionContainer : ConnectionContainer, IPacketHandler, IDisposable
    {
        ...
        {
            return
                $"{nameof(ClientConnectionContainer)}. TCP is alive {IsAlive_TCP}. UDP is alive {IsAlive_UDP}. Server IPAddress {IPAddress} Port {Port.ToString()}";
        }
        ...
        internal ClientConnectionContainer(string ipAddress, int port) : base(ipAddress, port)
        {
        ...
        /// <summary>
        ///     Est-ce que le client doit tenter de se connecter en UDP apres la connection TCP
        /// </summary>
        public bool AllowUDPConnection { get; set; }
        ...
        /// </summary>
        public TcpConnection TcpConnection => tcpConnection;

        /// <summary>
        ///     The current <see cref="Network.UdpConnection" /> for this instance.
        /// </summary>
        public UdpConnection UdpConnection => udpConnection;
        ...
        /// </summary>
        public bool IsAlive => IsAlive_TCP && IsAlive_UDP;
        ...
        /// <summary>
        ///     Signifies that a connection has been made on either the <see cref="TcpConnection" /> or
        ...
        /// <summary>
        ///     Signifies that a connection has been lost on either the <see cref="TcpConnection" /> or
        ///     <see cref="UdpConnection" />.
        /// </summary>
        ...
        /// <summary>
        ///     Initialises this <see cref="ClientConnectionContainer" /> instance and attempts to connect to the current
        ///     <see cref="Connection.IPRemoteEndPoint" />.
        /// </summary>
        ...
        private async void TryConnect()
        {
            ...
            if (!(tcpConnection is {IsAlive: true}))
                await OpenNewTCPConnection();
            if (!(udpConnection is {IsAlive: true}) && IsAlive_TCP)
        ...
        ///     Whether to call the <see cref="Connection.ConnectionClosed" /> event. <c>True</c> the
        ///     <see cref="ClientConnectionContainer" /> tries to reconnect to it's endpoint afterwards again; Plus, calles the
        ///     <see cref="Connection.ConnectionClosed" /> event. Otherwise <c>False</c>
        /// </param>
        ...
        {
            if (IsAlive_TCP) tcpConnection.RegisterStaticPacketHandler(handler);
            else tcpPacketHandlerBackup.RegisterStaticPacketHandler(handler);
        }
        ...
        {
            if (IsAlive_TCP) tcpConnection.RegisterPacketHandler(handler, obj);
        ...
        {
            if (IsAlive_UDP) udpConnection.RegisterStaticPacketHandler(handler);
            else udpPacketHandlerBackup.RegisterStaticPacketHandler(handler);
        }
        ...
        {
            if (IsAlive_UDP) udpConnection.RegisterPacketHandler(handler, obj);
        ...
        {
            TCP_RegisterStaticPacketHandler(handler);
            UDP_RegisterStaticPacketHandler(handler);
        }
        ...
        {
            TCP_RegisterPacketHandler(handler, obj);
            UDP_RegisterPacketHandler(handler, obj);
        }
        ...
        {
            var result = await CreateTcpConnection();
        ...
        {
            if (!AllowUDPConnection) return;
            var result = await CreateUdpConnection();
            ...
            udpConnection.ConnectionClosed += (closeReason, connection) =>
            {
                ...
                connectionLost?.Invoke(udpConnection, ConnectionType.UDP, closeReason);
                Reconnect();
            };
        ...
        public void SendPing()
        {
            if (tcpConnection != null && !tcpConnection.IsAlive)
            {
                if (ThrowExceptionOnUndeliverablePackets)
                    throw new ConnectionNotAliveException(tcpConnection,
                        "Can't send a ping, while the connection isn't alive.");
            }
            else
            {
        ...
        /// <summary>
        ///     Asynchronously sends the given <see cref="Packet" /> over the network via TCP and awaits a
        ...
        public void SendSlow(Packet packet)
        {
            if (tcpConnection == null || !tcpConnection.IsAlive)
            {
                if (ThrowExceptionOnUndeliverablePackets)
                    throw new ConnectionNotAliveException(tcpConnection,
                        $"Can't send a {nameof(Packet)}, while the connection isn't alive.");
            }
            else
            {
        ...
        public void SendSlow(Packet packet, object instance)
        {
            ...
            else if (ThrowExceptionOnUndeliverablePackets)
                throw new ConnectionNotAliveException(tcpConnection,
                    $"Can't send a {nameof(Packet)}, while the connection isn't alive.");
        }
        ...
        /// <summary>
        ///     Asynchronously sends the given <see cref="Packet" /> over the network via UDP and awaits a
        ///     <see cref="ResponsePacket" />
        ///     of the given type.
        /// </summary>
        /// <typeparam name="T">The type of <see cref="ResponsePacket" /> to await.</typeparam>
        ...
        {
            return await ConnectionFactory.CreateTcpConnectionAsync(IPAddress, Port);
        }
        ...
        {
            return await ConnectionFactory.CreateUdpConnectionAsync(tcpConnection);
        }
