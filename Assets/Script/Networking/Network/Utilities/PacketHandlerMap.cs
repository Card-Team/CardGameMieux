...
namespace Network.Utilities
{
    ...
    internal class PacketHandlerMap
    {
        ...
        /// </summary>
        private readonly ConcurrentDictionary<Type, Dictionary<object, int>> packetTypeToHandlerIdMap =
            new ConcurrentDictionary<Type, Dictionary<object, int>>();

        /// <summary>
        ///     Maps each packet id to a tuple, holding the packet handler method and the object on which the handler should be
        ///     called.
        /// </summary>
        private readonly Dictionary<int, (Delegate handlerDelegate, object handlerInstance)>
            packetIdToDelegateMethodMap =
        ...
        internal Delegate this[string key] =>
            keyToDelegateMethodMap.ContainsKey(key) ? keyToDelegateMethodMap[key] : null;
        ...
        internal Delegate this[Type packetType]
        {
            get
            {
                ...
                var typeHandlerId = packetTypeToHandlerIdMap[packetType].Values.First();
        ...
        internal int this[Type packetType, object handlerInstance] =>
            packetTypeToHandlerIdMap[packetType][handlerInstance];
        ...
        {
            var internalAssemblyTypes = Assembly.GetAssembly(typeof(PacketHandlerMap)).GetTypes();

            var externalTypes = map.packetTypeToHandlerIdMap.Keys.ToList()
                .Where(e => internalAssemblyTypes.All(i => i != e));

            foreach (var currentExternalType in externalTypes)
            {
                while (!packetTypeToHandlerIdMap.ContainsKey(currentExternalType) &&
                       !packetTypeToHandlerIdMap.TryAdd(currentExternalType,
                ...
                foreach (var currentExternalId in externalIds)
        ...
        {
            var packetType = typeof(P);

            Debug.Log($"Registering static packet handler for type {packetType}");
            if (packetTypeToHandlerIdMap.ContainsKey(packetType))
            {
        ...
        /// <param name="handlerDelegate">The delegate method to be invoked when the given packet is received.</param>
        /// <param name="handlerInstance">The handler object instance on which the delegate method will be invoked.</param>
        internal void RegisterPacketDelegate<P>(PacketReceivedHandler<P> handlerDelegate, object handlerInstance)
            where P : Packet
        {
            RegisterPacketHandler<P>(handlerDelegate, handlerInstance);
        }
        ...
        /// <summary>
        ///     Registers the given <see cref="PacketReceivedHandler{P}" /> method to be used for all packets of the given packet
        ///     type.
        /// </summary>
        /// <typeparam name="P">The type of packet for which the delegate method will be used.</typeparam>
        ...
        /// <summary>
        ///     Registers the given <see cref="Delegate" /> method to be used for all <see cref="RawData" /> packets that arrive
        ///     with the given key.
        /// </summary>
        /// <param name="key">The key that identifies the primitive type.</param>
        /// <param name="handlerDelegate">
        ///     The delegate method to invoke for incoming <see cref="RawData" /> packets with the given
        ///     key.
        /// </param>
        ...
        internal void UnRegisterPacketHandler<P>(object handlerInstance) where P : Packet
        {
            ...
            if ((!packetTypeToHandlerIdMap.ContainsKey(packetType) || handlerInstance != null) &&
            ...
            {
                var temporaryPacketId = packetTypeToHandlerIdMap[packetType].Values.ToList()[0];
            ...
            //Regular methods
            var packetId = packetTypeToHandlerIdMap[packetType][handlerInstance];
        ...
        internal void UnRegisterStaticPacketHandler<P>() where P : Packet
        {
            UnRegisterPacketHandler<P>(null);
        }
