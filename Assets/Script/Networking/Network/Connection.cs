...
{
    /// <summary>
    ///     Provides the basic methods that all <see cref="Connection" /> inheritors must implement. It ensures connectivity
    ///     and
    ///     keeps tracks of statistics such as latency. Multi-threaded with 3 separate threads per connection. After calling
    ///     the
    ///     <see cref="Close" /> method, every queued <see cref="Packet" /> will be sent before the <see cref="Connection" />
    ///     is fully
    ///     closed.
    /// </summary>
    /// <remarks>
    ...
    ///     <see cref="PacketReceivedHandler{P}" />.
    ...
    public abstract partial class Connection : IPacketHandler
    {
        ...
        /// <summary>
        ///     Whether this instance should send out a keep alive packet at specific intervals, to ensure there is an alive remote
        ///     connection.
        ...
        /// </summary>
        private readonly ConcurrentQueue<Tuple<Packet, object>> pendingUnknownPackets =
            new ConcurrentQueue<Tuple<Packet, object>>();
        ...
        /// <summary>
        ///     Holds all received <see cref="Packet" />s without a known <see cref="PacketReceivedHandler{P}" /> that are yet to
        ///     be
        ///     handled.
        /// </summary>
        ...
        /// </summary>
        private readonly ConcurrentQueue<Tuple<Packet, object>> sendPackets =
            new ConcurrentQueue<Tuple<Packet, object>>();
        ...
        /// <summary>
        ///     Maps a <see cref="RequestPacket" /> <see cref="Type" /> to the <see cref="Type" /> of the
        ///     <see cref="ResponsePacket" />
        ///     that handles it.
        /// </summary>
        private readonly Dictionary<Type, Type> requestResponseMap = new Dictionary<Type, Type>();

        /// <summary>
        ///     Maps <see cref="Packet" /> <see cref="Type" />s to the <see cref="PacketReceivedHandler{P}" /> that should be used
        ///     for
        ...
        ///     <see cref="RegisterPacketHandler{P}(PacketReceivedHandler{P}, object)" />, and
        ///     <see cref="RegisterRawDataHandler" />
        ...
        /// </summary>
        public bool IsAlive => readStreamThread.IsAlive && writeStreamThread.IsAlive && invokePacketThread.IsAlive &&
        ...
        /// <summary>
        ///     Gets or sets the performance of this <see cref="Connection" />. The higher the sleep intervals (the lower the
        ///     performance),
        ///     the slower this <see cref="Connection" /> will handle incoming, pending handling, and outgoing
        ///     <see cref="Packet" />s.
        /// </summary>
        ...
        /// </summary>
        public int IntPerformance => (int) Performance;
        ...
        ///     handle only a specific set of packets, a custom <see cref="IPacketConverter" /> can allow more throughput (no
        ///     slowdowns
        ...
        /// <summary>
        ///     Event signifying that a connection was closed between this <see cref="Connection" /> instance and another
        ///     <see cref="Connection" />.
        ///     This event is only visible for the network library itself. It garantuees, that the lib itself is capable of
        ...
        /// <summary>
        ///     Event signifying that this <see cref="Connection" /> instance established a new connection with either a
        ///     <see cref="TcpConnection" />
        ...
        {
            packetHandlerMap.UnRegisterStaticPacketHandler<T>();
        }

        /// <inheritdoc />
        public void UnRegisterPacketHandler<T>(object obj) where T : Packet
        {
            packetHandlerMap.UnRegisterPacketHandler<T>(obj);
        }
        ...
        {
            packetHandlerMap.UnRegisterStaticRawDataHandler(key);
        }

        #endregion Implementation of IPacketHandler

        /// <summary>
        ///     Registers all <see cref="Packet" /> inheritors in the given <see cref="Assembly" /> with this
        ///     <see cref="Connection" />.
        ///     Should this method be called manually, it must be called on both the server and client so that all
        ///     <see cref="Packet" />s
        ...
        ///     are not included, as the library will attempt to synchronise known <see cref="Packet" />s between the server and
        ///     client
        ///     automatically.
        /// </remarks>
        internal void AddExternalPackets(Assembly assembly)
        {
            assembly.GetTypes().ToList().Where(c => c.IsSubclassOf(typeof(Packet))).ToList().ForEach(p =>
            {
                ...
                var packetId = (ushort) Interlocked.Increment(ref currentTypeByteIndex);
                var packetTypeAttribute = p.GetCustomAttribute(typeof(PacketTypeAttribute));
                //Apply the local ID if there exist any.
                if (packetTypeAttribute != null) packetId = ((PacketTypeAttribute) packetTypeAttribute).Id;
                typeByte[p] = packetId;
            });

            assembly.GetTypes().ToList().Where(c => c.GetCustomAttributes(typeof(PacketRequestAttribute)).Any())
                .ToList().ForEach(c =>
                {
                    var requestAttribute =
                        (PacketRequestAttribute) c.GetCustomAttribute(typeof(PacketRequestAttribute));
        ...
        ///     connection.
        /// </returns>
        internal PacketHandlerMap BackupPacketHandler()
        {
        ...
        /// <returns>
        ///     A <see cref="Task{T}" /> representing the asynchronous operation, with the promise of a
        ///     <see cref="ResponsePacket" /> of
        ///     the given type upon completion.
        /// </returns>
        public async Task<T> SendAsync<T>(Packet packet) where T : ResponsePacket
        {
            return await new ChickenReceiver().Send<T>(packet, this);
        }

        /// <summary>
        ///     Serialises the given <see cref="Packet" /> using the current <see cref="PacketConverter" /> and queues it to be
        ///     sent
        ...
        public void Send(Packet packet, object instance)
        {
            ...
            if (!typeByte.ContainsKey(packet.GetType()) ||
                pendingUnknownPackets.Any(p => p.Item1.GetType().Assembly.Equals(packet.GetType().Assembly)))
            {
        ...
        private void ReadWork()
        {
            try
            {
                while (true)
                {
                    var packetType = BitConverter.ToUInt16(ReadBytes(2), 0);
                    var packetLength = BitConverter.ToInt32(ReadBytes(4), 0);
                    var packetData = ReadBytes(packetLength);
                    ...
                    var receivedPacket = packetConverter.GetPacket(typeByte[packetType], packetData);
        ...
        /// <summary>
        ///     Invokes the relevant <see cref="PacketReceivedHandler{P}" /> for each packet in the <see cref="receivedPackets" />
        ///     queue.
        /// </summary>
        private void InvokeWork()
        {
            try
            {
                while (true)
                {
                    WaitHandle.WaitAny(new[] {packetAvailableEvent, threadCancellationTokenSource.Token.WaitHandle});
        ...
        private void WriteSubWork()
        {
            while (sendPackets.Count > 0)
            {
                ...
                var packet = packetWithObject.Item1;
                ...
                var packetData = packetConverter.GetBytes(packet);
                var packetLength = BitConverter.GetBytes(packetData.Length);
                var packetByte = new byte[2 + packetLength.Length + packetData.Length];
        ...
        private void WriteWork()
        {
            try
            {
                while (true)
                {
                    WaitHandle.WaitAny(new[] {dataAvailableEvent, threadCancellationTokenSource.Token.WaitHandle},
                        TIMEOUT);
        ...
        private void HandleDefaultPackets(Packet packet)
        {
            ...
            if (packet.GetType().Equals(typeof(PingResponse)))
            {
                var elapsedTime = currentPingStopWatch.ElapsedMilliseconds;
                currentPingStopWatch.Reset();
            ...
            if (packet.GetType().Equals(typeof(CloseRequest)))
            {
                var closeRequest = (CloseRequest) packet;
                ExternalClose(closeRequest.CloseReason);
                return;
            }

            if (packet.GetType().Equals(typeof(EstablishUdpRequest)))
            {
                var localEndpoint = new IPEndPoint(IPAddress.IPv6Any, GetFreePort());
                var establishUdpRequest = (EstablishUdpRequest) packet;
                Send(new EstablishUdpResponse(localEndpoint.Port, establishUdpRequest));
                var udpConnection = CreateUdpConnection(localEndpoint,
            ...
            if (packet.GetType().Equals(typeof(EstablishUdpResponseACK)))
            {
            ...
            if (packet.GetType().Equals(typeof(AddPacketTypeRequest)))
            {
                var assembly = AppDomain.CurrentDomain.GetAssemblies()
                    .Where(a => a.FullName == ((AddPacketTypeRequest) packet).AssemblyName).SingleOrDefault();
            ...
            if (packet.GetType().Equals(typeof(AddPacketTypeResponse)))
            {
                var internalSendQueue = new List<Tuple<Packet, object>>();
                var addPacketTypeResponse = (AddPacketTypeResponse) packet;
            ...
            if (packet.GetType() == typeof(RawData))
            {
                var rawData = (RawData) packet;
                if (packetHandlerMap[rawData.Key] == null)
                    Logger.Log($"RawData packet has no listener. Key: {rawData.Key}", LogLevel.Warning);
                else packetHandlerMap[rawData.Key].DynamicInvoke(packet, this);
                return;
            }
            ...
            {
                Logger.Log("Provided delegate contains a bug. Packet invocation thread crashed.", exception,
                    LogLevel.Exception);
            }
        }

        /// <summary>
        ///     Invoked for any received <see cref="Packet" />s that don't have a registered
        ///     <see cref="PacketReceivedHandler{P}" />.
        /// </summary>
        ...
        protected virtual void PacketWithoutHandlerReceived(Packet packet)
        {
            ...
            if (receivedUnknownPacketHandlerPackets.Count < PacketBuffer)
                ...
                Logger.Log($"PacketBuffer exeeded the limit of {PacketBuffer}. " +
        ...
        /// <summary>
        ///     Whenever a <see cref="Packet" /> is received without a <see cref="PacketReceivedHandler{T}" /> already registered
        ///     for
        ///     it, it is placed in a timeout in the sad, lonely corner that is <see cref="receivedUnknownPacketHandlerPackets" />.
        ///     Whenever a <see cref="PacketReceivedHandler{P}" /> is registered via the 'Connection.RegisterXXX' methods, we need
        ///     to
        ...
        private void SearchAndInvokeUnknownHandlerPackets(Delegate del)
        {
            ...
            var delegateForPacketType = del.GetType().GenericTypeArguments.FirstOrDefault();
            if (receivedUnknownPacketHandlerPackets.Any(p => p.GetType().Equals(delegateForPacketType)))
            {
                var forwardToDelegatePackets =
                    receivedUnknownPacketHandlerPackets.Where(p => p.GetType().Equals(delegateForPacketType));

                foreach (var currentForwardPacket in forwardToDelegatePackets)
                {
                    Logger.Log(
                        $"Buffered packet {currentForwardPacket.GetType().Name} received a handler => Forwarding");
        ...
        ///     and writes all remaining queued <see cref="Packet" />s to the network (they are received before the
        ///     <see cref="CloseRequest" />
        ///     will be handled).
        /// </summary>
        /// <param name="closeReason">The reason for the <see cref="Connection" /> closing.</param>
        /// <param name="callCloseEvent">
        ///     If this <see cref="Connection" /> instance should call its <see cref="Connection" />
        ///     event.
        /// </param>
        public void Close(CloseReason closeReason, bool callCloseEvent = false)
        {
            ...
            {
                Logger.Log($"Couldn't send a close-message '{closeReason.ToString()}' to the endpoint.", exception,
                    LogLevel.Warning);
            }
        ...
        {
            Logger.Log("UnlockRemoteConnection will be removed in a future release.", LogLevel.Warning);
        }
        ...
        /// <summary>
        ///     Creates and returns a new <see cref="UdpConnection" />, with the given local endpoint, remote endpoint, and write
        ///     lock state.
        /// </summary>
        ...
        protected virtual UdpConnection CreateUdpConnection(IPEndPoint localEndPoint, IPEndPoint remoteEndPoint,
            bool writeLock)
        {
            return new UdpConnection(localEndPoint, remoteEndPoint, writeLock);
        }
        ...
        {
            return $"Local: {IPLocalEndPoint} Remote: {IPRemoteEndPoint}";
        }
    }
}