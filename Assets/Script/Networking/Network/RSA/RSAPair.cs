...
namespace Network.RSA
{
    ...
    public class RSAPair
    {
        ...
        public RSAPair(string publicKey, string privateKey, int keySize) : this(publicKey, privateKey, keySize,
            Connection.IsXpOrHigher)
        {
        }
        ...
        internal RSAPair(string publicKey, int keySize, bool eanbleOAEPadding) : this(publicKey, PRIVATE_KEY_UNKNOWN,
            keySize, eanbleOAEPadding)
        {
        }
        ...
        private RSAPair(string publicKey, string privateKey, int keySize, bool eanbleOAEPadding)
        {
            ...
            if (KeySize > 4096)
                Trace.TraceWarning(
                    $"{nameof(keySize)} exceeds the size of 4096 bits. Expect a major impact in performance.");
            ...
            encryptionByteSize = EnableOAEPadding ? (keySize - 384) / 8 + 6 : (keySize - 384) / 8 + 37;
        ...
        public bool EnableOAEPadding { get; }
