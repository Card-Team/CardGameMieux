...
namespace Network.RSA
{
    ...
    internal class RSAConnection : IPacketConverter, IRSACapability, IRSAConnection
    {
        ...
        public RSAConnection(Connection connection, RSAPair rsaPair)
        {
            ...
            RSACryptoServiceProviderExtensions.ImportParametersFromXmlString(DecryptionProvider, RSAPair.Private);
        ...
        /// </summary>
        private volatile bool isRSACommunicationActive;
        ...
        public bool IsOAEPaddingEnabled => IsRSACommunicationActive && RSAPair.EnableOAEPadding &&
                                           CommunicationPartnerRSAPair.EnableOAEPadding;
        ...
        {
            var isRSACommunicationActive = IsRSACommunicationActive;
            var unEncryptedData = PacketConverter.GetBytes(packet);

            var rsaStatus = BitConverter.GetBytes(isRSACommunicationActive);
            var packetData = isRSACommunicationActive ? EncryptBytes(unEncryptedData) : unEncryptedData;
            var packetDataWithStatus = new byte[packetData.Length + 1];
        ...
        {
            var isRSACommunicationActive = serialisedPacket[0] == 1;
            var data = new byte[serialisedPacket.Length - 1];
            Array.Copy(serialisedPacket, 1, data, 0, data.Length);

            return isRSACommunicationActive
                ? PacketConverter.GetPacket(packetType, DecryptBytes(data))
        ...
        private async void ExchangePublicKeys()
        {
            Connection.RegisterStaticPacketHandler<RSAKeyInformationRequest>((rsaKeyRequest, connection) =>
            {
                ...
                CommunicationPartnerRSAPair = new RSAPair(rsaKeyRequest.PublicKey, rsaKeyRequest.KeySize,
                    rsaKeyRequest.UseOAEPadding);
                EncryptionProvider = new RSACryptoServiceProvider(CommunicationPartnerRSAPair.KeySize);
                RSACryptoServiceProviderExtensions.ImportParametersFromXmlString(EncryptionProvider,
                    CommunicationPartnerRSAPair.Public);

                connection.Send(new RSAKeyInformationResponse(RSAPair.Public, RSAPair.KeySize, rsaKeyRequest));
            });

            var keyInformationResponse =
                await Connection
                    .SendAsync<RSAKeyInformationResponse>(new RSAKeyInformationRequest(RSAPair.Public, RSAPair.KeySize,
                        RSAPair.EnableOAEPadding)).ConfigureAwait(false);

            Connection.Logger.Log($"{Connection.GetType().Name} RSA Encryption active.");
        ...
        public byte[] DecryptBytes(byte[] bytes)
        {
            ...
            for (var currentIndex = 0; currentIndex < bytes.Length / RSAPair.DecryptionByteSize; currentIndex++)
                chunkData.Add(bytes.Skip(currentIndex * RSAPair.DecryptionByteSize).Take(RSAPair.DecryptionByteSize)
                    .ToArray());
        ...
        public byte[] EncryptBytes(byte[] bytes)
        {
            ...
            for (var currentIndex = 0;
                 currentIndex <= bytes.Length / remoteRSAKeyPair.EncryptionByteSize;
                 currentIndex++)
                chunkData.Add(bytes.Skip(currentIndex * remoteRSAKeyPair.EncryptionByteSize)
                    .Take(remoteRSAKeyPair.EncryptionByteSize).ToArray());
