...
namespace Network.Async
{
    ...
    internal class ChickenReceiver : IDisposable
    {
        ...
        /// <summary>
        ///     A <see cref="ManualResetEvent" /> that allows for the instance to wait until the <see cref="ResponsePacket" /> for
        ///     a sent packet is received.
        /// </summary>
        ...
        public async Task<R> Send<R>(Packet packet, Connection connection) where R : ResponsePacket
        {
            ...
            catch (OverflowException overflowException)
            {
                connection.Logger.Log(
                    $"Exception while waiting for async packet occured. Request packet {packet.GetType().Name}",
                    overflowException, LogLevel.Error);
            }

            //No answer from the endPoint
            if (receivedAsyncPacket == null)
            {
                var emptyPacket = PacketConverterHelper.InstantiateGenericPacket<R>();
                emptyPacket.State = PacketState.Timeout;
                return emptyPacket;
            }
